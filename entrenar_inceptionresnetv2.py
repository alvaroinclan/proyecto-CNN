"""
==============================================================================
Entrenamiento y Validación del Modelo InceptionResNetV2 - Dataset LC25000
==============================================================================

Descripción:
    Este script entrena y evalúa un modelo InceptionResNetV2 (Transfer Learning)
    sobre las imágenes LBP del dataset LC25000 (64x64 píxeles).

    Arquitectura (basada en Alsubai, 2024):
      - InceptionResNetV2 preentrenada en ImageNet (sin capa top)
    ...
    Nota sobre InceptionResNetV2:
      El input mínimo recomendado por Keras para InceptionResNetV2 es 75×75.
      Con imágenes de 64×64, se utiliza una capa de upsampling (UpSampling2D)
      para escalar las imágenes a 75×75 antes de pasarlas al modelo base.
      Esto permite usar el modelo preentrenado sin perder la compatibilidad
      con los pesos de ImageNet.
...
from tensorflow.keras.layers import (
    Dense, Dropout, BatchNormalization, Input, UpSampling2D,
)
...
# InceptionResNetV2 requiere un input mínimo de 75×75
INCEPTION_MIN_SIZE = 75
...
NOMBRE_DIR_RESULTADOS = "resultados_inceptionresnetv2"

# Hiperparámetros
BATCH_SIZE = 32  # Más pequeño que ResNet/CNN por el mayor tamaño del modelo
EPOCHS = 50
LEARNING_RATE = 0.001
...
# =============================================================================
# 4. Construcción del modelo InceptionResNetV2
# =============================================================================

def construir_modelo_inceptionresnetv2(input_shape: tuple = IMG_SHAPE) -> Model:
    """
    Construye el modelo InceptionResNetV2 con transfer learning.

    Arquitectura (siguiendo el patrón del paper de Alsubai, 2024):
      - Input (64, 64, 3)
      - UpSampling2D para escalar a 75×75 (mínimo requerido por InceptionResNetV2)
      - InceptionResNetV2 preentrenada en ImageNet (include_top=False, pooling='max')
      - BatchNormalization
    ...
    Nota:
      InceptionResNetV2 espera un input mínimo de 75×75. Como nuestras imágenes
      son 64×64, usamos UpSampling2D con interpolación bilineal para escalarlas
      a 75×75 antes de alimentar el modelo base. Esto es una adaptación necesaria
      por la reducción de resolución que realizamos.
    ...
    -------
    Model
        Modelo InceptionResNetV2 compilado.
    """
    # Input layer con la resolución original de nuestras imágenes (64×64)
    inputs = Input(shape=input_shape)

    # Escalar las imágenes a 75×75 para cumplir con el mínimo de InceptionResNetV2
    # Usamos tf.image.resize para un redimensionado flexible con interpolación bilineal
    x = tf.keras.layers.Lambda(
        lambda img: tf.image.resize(img, (INCEPTION_MIN_SIZE, INCEPTION_MIN_SIZE)),
        name="resize_to_75x75",
    )(inputs)

    # Base model: InceptionResNetV2 preentrenada en ImageNet
    base_model = tf.keras.applications.InceptionResNetV2(
        include_top=False,
        weights="imagenet",
        input_shape=(INCEPTION_MIN_SIZE, INCEPTION_MIN_SIZE, IMG_CHANNELS),
        pooling="max",
    )

    # Dejar todas las capas entrenables (fine-tuning completo)
    base_model.trainable = True

    # Pasar las imágenes redimensionadas por el modelo base
    x = base_model(x, training=True)

    # Cabeza de clasificación (igual que en ResNet50)
...
def entrenar_modelo(
    ...
    """
    Entrena el modelo InceptionResNetV2 con callbacks de checkpoint y early stopping.
    Si existe un checkpoint previo, lo carga y reanuda el entrenamiento.
    """
    os.makedirs(ruta_resultados, exist_ok=True)

    ruta_checkpoint = os.path.join(
        ruta_resultados, "inceptionresnetv2_mejor_modelo.keras"
    )
    ruta_historial = os.path.join(
        ruta_resultados, "inceptionresnetv2_historial.json"
    )
    ruta_modelo_final = os.path.join(
        ruta_resultados, "inceptionresnetv2_modelo_final.keras"
    )
    ...
    print(f"\n{'=' * 70}")
    print(f"  ENTRENAMIENTO DEL MODELO INCEPTIONRESNETV2")
    print(f"{'=' * 70}")
    ...
    print(f"  Input resize:       {IMG_HEIGHT}×{IMG_WIDTH} → "
          f"{INCEPTION_MIN_SIZE}×{INCEPTION_MIN_SIZE}")
    print(f"{'=' * 70}\n")
...
def evaluar_modelo(
    ...
    """
    print(f"\n{'=' * 70}")
    print(f"  EVALUACIÓN DEL MODELO INCEPTIONRESNETV2 EN TEST SET")
    print(f"{'=' * 70}")
    ...
    # Comparación con el paper (InceptionResNetV2 + LBP, 768×768)
    print(f"\n  Comparación con el paper (InceptionResNetV2 + LBP, 768×768):")
    ...
    for metrica in paper:
        ...
        print(f"  {metrica:<12s} {paper[metrica]:>9.2f}% "
              f"{nuestro[metrica]:>9.2f}% {signo}{delta:>8.2f}%")
    ...
    for i, clase in enumerate(CLASES):
        row = (f"  {clase[:12]:>12s}"
               + "".join(f" {cm[i, j]:>8d}" for j in range(NUM_CLASES)))
        print(row)
    ...
    ruta_metricas = os.path.join(
        ruta_resultados, "inceptionresnetv2_metricas.json"
    )
...
def main(ruta_base: str = None):
    """
    Función principal: carga datos, construye InceptionResNetV2, entrena y evalúa.
    """
    print("=" * 70)
    print("  ENTRENAMIENTO Y EVALUACIÓN - MODELO INCEPTIONRESNETV2")
    ...
    # 2. Construir modelo InceptionResNetV2
    print(f"\n{'=' * 70}")
    print(f"  ARQUITECTURA DEL MODELO INCEPTIONRESNETV2")
    print(f"{'=' * 70}")
    model = construir_modelo_inceptionresnetv2()
    ...
    # Resumen final
    print(f"\n{'=' * 70}")
    print(f"  RESUMEN FINAL - MODELO INCEPTIONRESNETV2")
    print(f"{'=' * 70}")
    ...
    print(f"  - inceptionresnetv2_mejor_modelo.keras  (mejor checkpoint)")
    print(f"  - inceptionresnetv2_modelo_final.keras  (modelo final)")
    print(f"  - inceptionresnetv2_historial.json      (curvas de aprendizaje)")
    print(f"  - inceptionresnetv2_metricas.json       (métricas de evaluación)")
